$(() => {
  ...
  const controller = {
    ...
    matchFilenames: () => {
      ...
      model
        .matchFilenames(controller.loadedThumblist, fuseOptions)
        .then((result) => {
          view.hideLoading();
    ...
    readDirectory: (result) => {
      if (!result.canceled) {
        return model.readDirectory(result.filePaths[0]).then((filelist) => {
          return {
            filelist: filelist,
    ...
    handleSaveNoThumbPlaylist: () => {
      ...
      let path = controller.playlistPath.substring(
        0,
        controller.playlistPath.lastIndexOf(".")
      );
      path += "_nothumb.lpl";
      model
        .savePlaylist(list, title, path)
        .then((ret) => {
          console.log(ret);
        })
        .catch((error) => {
    ...
    setThreshold: (value) => {
      view.$ipt_threshold.val(value * 10).trigger("input");
    },
    setDefaultConfig: () => {
      controller.setThreshold(0.6);
    },
    setPackListMenu: async () => {
      try {
        const list = [];
        const res = await model.getThumbnailsPacksList();
        if (res) {
          const $body = $(res);
          ...
          model.setPackListMenu(list);
        }
      } catch (error) {
        model.setPackListMenu([]);
  ...
  const view = {
    init: () => {
      ...
      view.$btn_download = $("#btn-download");
    ...
    renderPlaylistTable: () => {
      ...
      controller.loadedPlaylist.forEach((row, idx) => {
        ...
        score =
          row.thumbnail && row.thumbnail.score
            ? row.thumbnail.score.toFixed(2)
            : 0;
        thumbimg = row.thumbnail
          ? `<img src="${row.thumbnail.path}" class="thumbimg"/>`
          : "-";
        btngroup = `<div class="btn-group">
          ...
          <button class="btn btn-mini btn-negative btn-delete" data-idx="${idx}" ${
          !row.thumbnail ? 'style="display:none;"' : ""
        }>
